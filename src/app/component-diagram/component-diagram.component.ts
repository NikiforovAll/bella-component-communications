...
import { GraphData, ComponentBuilder, SVGConfig, ComponentDiagramGraphLayout} from "graph-builder";
...
export class ComponentDiagramComponent implements OnInit {
  ...
  constructor(
    private storageService: StorageService, private _elementRef: ElementRef) {
  ...
  public drawComponentDiagram($event) {
    if ($event.index == 1) {
      let svgDOM = this._elementRef.nativeElement.querySelector("#main-component-diagram");
      var builder = new ComponentBuilder(this.svgConfig, svgDOM);
      builder.clear();
      let dataToRender = this.filterDataByComponentName(
        this.data,
