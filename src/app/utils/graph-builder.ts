...
class GraphBuilder {
    ...
    private initializeDOM(data: GraphData): void {
        ...
        const radius = this.svgConfig.height / 2 - this.svgConfig.margin;
        const sidesCount: number = data.nodes.length;
        const coordinates = polygon(center.x, center.y, radius, sidesCount);
    ...
    private drawNode(component: DiagramComponent): void {
        ...
        const container = this.svg.append('g').attr('transform', this.buildTransformTemplate(component.dom.x, component.dom.y));
